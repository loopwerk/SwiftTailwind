...
- **SwiftTailwind.swift** — Public API. `struct SwiftTailwind` takes a Tailwind version, auto-detects the project root by walking up from `#file` to find `Package.swift`, and exposes `run(input:output:options:)` which downloads the binary (if needed) and executes it via `Foundation.Process`.
